...
- (void)playingWithSeconds {
    ...
    if (self.playingDelegate && [self.playingDelegate respondsToSelector:@selector(playerPlayingWithTime:)]) {
        // 计算播放器现在播放的秒数
        NSTimeInterval time = self.player.currentTime.value / self.player.currentTime.timescale;
//        NSLog(@"当前1:%lld",self.player.currentTime.value);
//        NSLog(@"当前2:%d",self.player.currentTime.timescale);
        [self.playingDelegate playerPlayingWithTime:time];
    }
}
